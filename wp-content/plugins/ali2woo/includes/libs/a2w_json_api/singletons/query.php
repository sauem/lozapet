...
if (!class_exists('A2W_JSON_API_Query')) {

    class A2W_JSON_API_Query {
        ...
        function get_legacy_controller($json) {
            ...
            if ($json == 'submit_comment') {
                if ($a2w_json_api->controller_is_active('respond')) {
                    return 'respond';
                } else {
                    $a2w_json_api->error("The 'submit_comment' method has been removed from the Core controller. To use this method you must enable the Respond controller from WP Admin > Settings > JSON API.");
                }
            } else if ($json == 'create_post') {
                if ($a2w_json_api->controller_is_active('posts')) {
                    return 'posts';
                } else {
                    $a2w_json_api->error("The 'create_post' method has been removed from the Core controller. To use this method you must enable the Posts controller from WP Admin > Settings > JSON API.");
                }
            } else {
                return 'core';
            }
        ...
        function get_method($controller) {
            ...
            if (empty($method)) {
                ...
                return false;
            } else if (method_exists("A2W_JSON_API_{$controller}_Controller", $method)) {
                // Case 2: an explicit method was specified
                return $method;
            } else if ($controller == 'core') {
                $a2w_json_api->error("Unknown method '$method'.");
                return 'info';
            }
